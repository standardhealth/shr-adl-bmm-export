...
class AdlElement {
  ...
  constructConstraintDefinitions(de) {
    ...
    for (const f of this.combinedFieldsAndValue) {
      ...
      if (f.inheritedFrom) {
        var hierarchialDepth = de.hierarchy.length;
      }
      ...
      const findOrCreateTermDefByIdentifier = (identifier) => {
        ...
        if (!termDef || termDef.isSourceDef()) {
          const numbering = this.termDefinitions.id.filter(td=>!td.isInherited).map(td => td.nodeId).slice(-1)[0].slice(-1)[0] + 1;
          var desc = '-';
      ...
      for (const c of f.constraintsFilter.type.constraints) {
        if (c.lastModifiedBy.equals(de.identifier)) {
          const onValue = c.onValue || (fieldOrigDef.value && fieldOrigDef.value.identifier.fqn == c.path[0].fqn);
          cstBase.subConstraints.push(new ADLTypeConstraint(termDefField, findOrCreateTermDefByIdentifier(c.isA), onValue, c.path));
        }
      }
...
class AbstractADLConstraint {
  constructor(constrainedTerm, constraintDef, path) {
...
class ADLTypeConstraint extends AbstractADLConstraint {
  constructor(constrainedField, constraintDef, onValue, path) {
    super(constrainedField, constraintDef, path);
