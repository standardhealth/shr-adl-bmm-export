...
class DataElementADLFormatter {
  ...
  formatMetaData(el) {
    ...
    const formatOriginalAuthor = () => {
      ...
      return [
        `\toriginal_author = <`,
        `\t\t["name"] = <"${this.config.provenanceInfo.leadAuthor.name}">`,
        `\t\t["organisation"] = <"${this.config.provenanceInfo.leadAuthor.organization}">`,
        `\t\t["email"] = <"${this.config.provenanceInfo.leadAuthor.email}">`,
        `\t\t["date"] = <"${date}">`,
        `\t>`
      ].join('\n');
    };
    ...
    const formatAdditionalInformation = () => {
      ...
      if (this.config.provenanceInfo.otherAuthors) {
        additionalInformation.push(`\tother_contributors = <"${this.config.provenanceInfo.otherAuthors.map(a => `${a.name} <${a.email}>`).join(', ')}">`);
      }
      ...
      if (this.config.provenanceInfo.copyright) {
        additionalInformation.push(`\tcopyright = <"${this.config.provenanceInfo.copyright}">`);
      }
      if (this.config.provenanceInfo.ipAcknowledgements) {
