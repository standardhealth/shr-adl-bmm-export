...
class AdlElement {
  ...
  constructIdTermDefs(de) {
    ...
    if (newValueDef && !newValueDef.inheritance && newValueDef.identifier) {
      const fieldElementDefinition = this.specs.dataElements.findByIdentifier(newValueDef.identifier);
      const adlField = new ADLField(fieldElementDefinition, true);
      ...
      // }
      const atFieldDef = new TermDefinition(sourceValue.identifier.name, sourceValue.description, [...Array(hierarchialDepth - 1).fill(0), numbering++], 'id', newValueDef);
...
class DataElementADLFormatter {
  ...
  formatConstraintsOnFields(el) {
    ...
    const formattedDeclaration = `${declarativeConstraintTerm.name}${formatId(declarativeConstraintTerm.nodeId, declarativeConstraintTerm.type)}`;

    const formattedConstraints = [];

    for (const c of el.termConstraints) {
      const name = (c.constrainedTerm.constructor.name == 'ADLField') ?
        (c.constrainedTerm.isValue) ? 'value' : c.constrainedTerm.identifier.name
        : c.constrainedTerm.name;
