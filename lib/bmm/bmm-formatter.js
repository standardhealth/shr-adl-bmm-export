  ...
  formatPackages() {
    ...
    for (const ns in this.bmmSpecs.packages) {
      ...
      const formattedPkg = [
        `["${ns}"] = <`,
        `   name = <"${ns}">`,
        `   classes = <${pkgs.map(de => `"${de.identifier.name}"`.replace(/-/,'')).join(', ')}>`,
        `>`
      ];
  ...
  formatDefinitions() {
    ...
    for (const de in this.bmmSpecs.definitions) {
      ...
      if (def.ancestors.length > 0) {
        formattedDef.push(`\tancestors = <${def.ancestors.map(a => `"${a.name}"`.replace(/-/,'')).join(',')}, ...>`);
      }
  ...
  formatProperties(properties) {
    ...
    for (const p in properties) {
      ...
      const formattedProperty = [
        `["${prop.name.replace(/-/,'')}"] = (${prop.p_bmm_type}) <`,
